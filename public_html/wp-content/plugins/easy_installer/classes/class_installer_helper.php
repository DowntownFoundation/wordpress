...
class EASYFInstallerHelper {
	...
	private function getMetadataValue($option = '') {
		global $config_suboption;
		$array = get_object_vars($easy_metadata['data']);
		if(isset($array[$option.$config_suboption]) && !empty($array[$option.$config_suboption])) {
			return $array[$option.$config_suboption];
		}
	}
